"""
Unified patient schema shared between the clinical trials pipeline and
the lab report upload API.

Both systems extract the same JSON structure so that patient data from a
PDF lab report can be directly compared against clinical trial requirements.

Five categories:
  1. Patient demographics (age, race, anthropometrics, location)
  2. Pregnancy status (current pregnancy or postpartum info)
  3. Health history (smoking, breastfeeding)
  4. Clinical diagnoses (boolean flags)
  5. Lab biomarkers (numeric value + unit + high-risk flag)
  6. Vital signs (numeric value + unit + flag level)
"""

from typing import Any

# ---------------------------------------------------------------------------
# Schema definitions (for documentation / reference only)
# ---------------------------------------------------------------------------

PATIENT_DEMOGRAPHICS = {
    "age": "Patient age in years (integer)",
    "race_ethnicity": "List of race/ethnicity identifiers (e.g. ['white', 'hispanic'])",
...
PREGNANCY_STATUS = {
    "currently_pregnant": "Whether the patient is currently pregnant (true/false)",
    "current_week": "Current gestational week if pregnant (integer or null)",
    "pregnancy_type": "'single' or 'multiple' (twins/triplets) or null",
    "delivery_date": "Date of most recent delivery if postpartum (ISO date string or null)",
    "delivery_type": "'single' or 'multiple' for most recent delivery or null",
}

HEALTH_HISTORY = {
    "currently_breastfeeding": "Whether currently breastfeeding (true/false/null)",
    "smoking_status": "'never', 'former', or 'current' (or null if unknown)",
}

CLINICAL_DIAGNOSES = {
    "preeclampsia": "Pregnancy-related hypertension with organ damage",
    "preeclampsia_onset": "early (<34 wks) / late (>=34 wks) / null",
    "hellp_syndrome": "Hemolysis, elevated liver enzymes, low platelets",
    "gestational_hypertension": "Pregnancy-induced hypertension without proteinuria",
    "peripartum_cardiomyopathy": "Pregnancy-induced heart failure (PPCM)",
    "gestational_diabetes": "Glucose intolerance developing during pregnancy",
    "preterm_delivery": "Delivery before 37 weeks gestation",
    "delivery_week": "Gestational week of delivery (integer or null)",
}

LAB_BIOMARKERS = {
    "sflt1_plgf_ratio": {"description": "Preeclampsia predictor (placental imbalance)", "threshold": "> 38", "unit": "ratio"},
    "nt_probnp": {"description": "Cardiac stress marker", "threshold": "> 125", "unit": "pg/mL"},
    "troponin_t": {"description": "Cardiac damage marker", "threshold": "> 14", "unit": "ng/L"},
    "proteinuria": {"description": "Kidney strain from hypertension", "threshold": "> 300 mg/24h or >= 1+ dipstick", "unit": "mg/24h"},
    "hba1c": {"description": "3-month average blood sugar", "threshold": ">= 5.7", "unit": "%"},
    "hemoglobin": {"description": "Oxygen-carrying protein in blood", "threshold": None, "unit": "g/dL"},
    "fasting_glucose": {"description": "Blood sugar after fasting", "threshold": None, "unit": "mg/dL"},
    "total_cholesterol": {"description": "Total blood cholesterol level", "threshold": None, "unit": "mg/dL"},
}

VITAL_SIGNS = {
    "systolic_bp": {"description": "Top blood pressure number", "flag_threshold": 140, "severe_threshold": 160, "unit": "mmHg"},
    "diastolic_bp": {"description": "Bottom blood pressure number", "flag_threshold": 90, "severe_threshold": 110, "unit": "mmHg"},
    "resting_heart_rate": {"description": "Resting pulse rate", "flag_threshold": 100, "severe_threshold": None, "unit": "bpm"},
}

# Top-level keys the LLM response must contain.
UNIFIED_EXPECTED_KEYS = {"demographics", "pregnancy", "health_history", "diagnoses", "biomarkers", "vitals"}

# ---------------------------------------------------------------------------
# JSON schema example (embedded in prompts for the LLM)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Shared extraction rules (appended to both prompts)
# ---------------------------------------------------------------------------
...
def validate_biomarker_response(parsed: Any) -> dict[str, Any] | None:
    """Validate that a parsed LLM response matches the unified schema.

    Returns the dict if valid, or None if the structure is wrong.
    """
    ...
    if not isinstance(parsed.get("demographics"), dict):
        return None
    if not isinstance(parsed.get("pregnancy"), dict):
        return None
    if not isinstance(parsed.get("health_history"), dict):
        return None
    if not isinstance(parsed.get("diagnoses"), dict):
        return None
    if not isinstance(parsed.get("biomarkers"), dict):
        return None
    if not isinstance(parsed.get("vitals"), dict):
        return None
